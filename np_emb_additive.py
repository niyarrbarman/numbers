"""
Number Embedding Representation — Additive Loss Variant
========================================================
PyTorch implementation (GPU-accelerated).

Same architecture as np_emb_torch.py (v8), but adds an **additivity loss**
during training that encourages:

    e(x + y) ≈ e(x) + e(y)

This is Approach 2 from the analysis: the encoder architecture is unchanged,
but an additional loss term penalises ||e(x+y) - (e(x) + e(y))||² over
random pairs sampled from each training batch.

The additivity loss is ramped in during training to avoid interfering with
early reconstruction learning:
  - Steps 0 to add_start (20%): no additivity loss
  - Steps add_start to add_end (30%): linear ramp
  - Steps add_end to end: full weight

Theoretical note: the only continuous f: R → R^d with exact f(x+y) = f(x)+f(y)
is f(x) = x*v (a linear map). So exact additivity collapses the embedding to 1D.
This loss encourages *approximate* additivity while letting the other loss terms
maintain expressiveness and reversibility.

Usage:
    python np_emb_additive.py --max-steps 500000 --add-weight 0.1
"""
...
class NumberEncoder(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        log_norm = torch.log(proj_norm + 1e-8)
        ...
        return torch.cat([normed, log_norm], dim=-1)


# =============================================================================
# Decoder (identical to v8)
# =============================================================================
...
# =============================================================================
# Complete System (with additivity loss)
# =============================================================================
...
class NumberEmbeddingSystem(nn.Module):
    ...
    def compute_loss(self, x: Tensor, emb: Tensor, recon: Tensor,
        ...
        # Term 6: Additivity loss — ||e(x+y) - (e(x) + e(y))||²
        if lam_add > 0:
            half = n // 2
            x1, x2 = x[:half], x[half:2 * half]
            emb1, emb2 = emb[:half], emb[half:2 * half]
            # Forward pass for the sum values
            emb_sum_actual = self.encoder(x1 + x2)
            emb_sum_target = emb1 + emb2
            loss_add = F.mse_loss(emb_sum_actual, emb_sum_target)
        else:
            loss_add = torch.tensor(0.0, device=emb.device)
    ...
    def train_model(self, num_steps: int = 500000, batch_size: int = 512,
        ...
        # Additivity loss ramp: 20-30% of training
        add_start = int(num_steps * 0.20)
        add_end = int(num_steps * 0.30)

        self.train()
        for step in range(1, num_steps + 1):
            # Phase-2 lambda ramp for relative MSE
            ...
            # Additivity loss ramp
            if step < add_start:
                lam_add = 0.0
            elif step < add_end:
                lam_add = add_weight * (step - add_start) / (add_end - add_start)
            else:
                lam_add = add_weight
            ...
            loss = self.compute_loss(x, emb, recon, sign_logit, lam_rel, lam_add)
            loss.backward()
            ...
            if step % log_interval == 0:
                ...
                add_tag = f" add={lam_add:.3f}" if lam_add > 0 else ""
                print(f"  Step {step:>6d}/{num_steps} | Loss: {avg:.6f} [{phase}{add_tag}] | lr: {lr_t:.2e}")
...
def test_additivity(system: NumberEmbeddingSystem):
    """Test how well e(x+y) ≈ e(x) + e(y) holds."""
    print("-" * 70)
    ...
    for name, lo1, hi1, lo2, hi2 in ranges:
        ...
        # For "integers" range, round to ints
        if name == "integers":
            x1 = x1.round()
            x2 = x2.round()
        ...
        with torch.no_grad():
            ...
            diff = (e_sum - e_target).norm(dim=-1)
            ref = e_sum.norm(dim=-1).clamp(min=1e-8)
            rel_err = diff / ref
            abs_err = diff
        ...
        all_rel.append(mean_rel)
        print(f"  {name:>20}: rel_err={mean_rel:.4f} (max {max_rel:.4f}), abs_err={mean_abs:.4f}")

    overall = sum(all_rel) / len(all_rel)
    print(f"\n  Overall mean relative additivity error: {overall:.4f}")
    return overall
...
def run_tests(system: Optional[NumberEmbeddingSystem] = None, device: torch.device = None):
    ...
    # === TEST 6: ADDITIVITY ===
    add_err = test_additivity(system)
    check("Mean additivity error < 0.5", add_err < 0.5, f"Error: {add_err:.4f}")
...
def demo(num_steps: int = 500000, add_weight: float = 0.1,
         device: torch.device = None):
    if device is None:
        device = get_device()
    print("=" * 70)
    print("NUMBER EMBEDDING — ADDITIVE LOSS VARIANT")
    print("=" * 70)
    print()
    ...
    print(f"  Additivity weight: {add_weight}")
    print()
    ...
    print(f"Training ({num_steps} steps, batch 512, add_weight={add_weight})...")
    t0 = time.time()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Number Embedding — Additive Loss Variant")
    parser.add_argument("--max-steps", type=int, default=500000,
                        help="Training steps (default: 500000)")
    parser.add_argument("--add-weight", type=float, default=0.1,
                        help="Additivity loss weight (default: 0.1)")
    ...
    log_path = os.path.join("logs",
                            f"run_addloss_{time.strftime('%Y%m%d_%H%M%S')}_w{args.add_weight}.log")
    ...
    print(f"Additivity weight: {args.add_weight}")
    if torch.cuda.is_available():
    ...
    try:
        if args.test_only:
            ...
            system = demo(num_steps=args.max_steps, add_weight=args.add_weight,
                          device=device)
            save_checkpoint(system, args.max_steps, args.add_weight)
        else:
            system = demo(num_steps=args.max_steps, add_weight=args.add_weight,
                          device=device)
            save_checkpoint(system, args.max_steps, args.add_weight)
