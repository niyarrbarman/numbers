#!/bin/bash
#
# Resume the surface pipeline from the current best Stage-1 checkpoint.
# This skips Stage 1 entirely and submits:
#   1. Stage 2 surface LoRA training
#   2. Zero-shot synth benchmark vs the existing base checkpoint
#
set -euo pipefail
mkdir -p slurm
...
S1_CKPT="/tmpdir/m24047brmn/numbers/model_checkpoints/luciole_surface_s1/ckpt_best.pt"
...
if [[ ! -f "${S1_CKPT}" ]]; then
    echo "Missing Stage 1 best checkpoint: ${S1_CKPT}" >&2
    exit 1
fi
...
if [[ ! -f "${S2_ADAPTED_DATA}/train.bin" || ! -f "${S2_ADAPTED_DATA}/train_surface.bin" ]]; then
    echo "Missing adapted Stage 2 data under: ${S2_ADAPTED_DATA}" >&2
    exit 1
fi

if [[ ! -f "${S2_BASE_DATA}/test_examples.json" || ! -f "${S2_ADAPTED_DATA}/test_examples.json" ]]; then
    echo "Missing synth benchmark test examples under: ${S2_DATA_DIR}" >&2
    exit 1
fi
...
echo "=========================================================="
echo "Surface Resume From S1 Best"
...
JOB_S2_TRAIN=$(sbatch --parsable <<EOF
#!/bin/bash
...
echo "  [1] Stage 2 train: ${JOB_S2_TRAIN}"
