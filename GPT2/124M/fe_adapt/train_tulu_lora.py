"""Unified LoRA fine-tuning on tulu-3-sft-personas-math-grade.

Supports two modes controlled by `use_adapter`:
  False: Base LoRA  -- pretrained Luciole + LoRA, no NumberEncoder
  True:  Adapted    -- Stage 1 checkpoint (NumberEncoder adapter) + LoRA

Same hyperparams, same training loop. Only difference: init source and data dir.

Usage:
  # Base LoRA
  python train_tulu_lora.py use_adapter=False pretrained_ckpt=... data_dir=.../base

  # Adapted LoRA
  python train_tulu_lora.py use_adapter=True stage1_ckpt=... data_dir=.../adapted

  # Resume either
  python train_tulu_lora.py init_from=resume resume_ckpt=...
"""
...
# Mode
use_adapter = True   # False = base LoRA, True = adapted (Stage 1 + LoRA)

# I/O
pretrained_ckpt = ''  # for use_adapter=False: converted Nemotron checkpoint
stage1_ckpt = ''      # for use_adapter=True: Stage 1 checkpoint with trained adapter
out_dir = '/tmpdir/m24047brmn/numbers/model_checkpoints/tulu_lora'
eval_interval = 2000
log_interval = 10
diag_interval = 100
...
mode_str = "ADAPTED (Stage1 + LoRA)" if use_adapter else "BASE (LoRA only)"
...
if init_from == 'fresh':
    if use_adapter:
        # --- Adapted: load Stage 1 checkpoint ---
        assert stage1_ckpt, "stage1_ckpt required when use_adapter=True"
        ...
        model_args['num_emb_checkpoint'] = ''  # encoder in state dict
        ...
        # --- Base: load pretrained Nemotron ---
        assert pretrained_ckpt, "pretrained_ckpt required when use_adapter=False"
        print(f"Loading pretrained checkpoint: {pretrained_ckpt}")
        model_args = dict(
            n_layer=12, n_head=24, n_kv_head=8,
            n_embd=768, ffn_hidden=3072,
            block_size=block_size, bias=False,
            vocab_size=50258, dropout=0.0,
            num_emb_dim=128, num_emb_checkpoint='',
            ...
            num_blend_beta_infer=1.0,
        )
        nemconf = NemotronConfig(**model_args)
        model = Nemotron(nemconf)
        model.load_pretrained_nemotron(pretrained_ckpt)

    # Apply LoRA AFTER loading weights
    ...
    for name, p in model.named_parameters():
        ...
        elif use_adapter and name.startswith('num_adapter.'):
            p.requires_grad = True
        else:
            p.requires_grad = False
    ...
    model_args['block_size'] = block_size
    use_adapter = checkpoint['config'].get('use_adapter', use_adapter)
...
print(f"Parameter budget ({mode_str}):")
...
def eval_samples(max_samples=3):
    ...
    for b in range(X.size(0)):
        ...
        for seg_idx in range(len(eot_positions)):
            ...
            # Find "Assistant:" in context to split input/output
            assistant_pos = None
    ...
    if iter_num % diag_interval == 0 and master_process:
        ...
        if use_adapter and _num_inj is not None:
            print(f"  num inject: base_norm {_num_inj['base_norm']:.4f}, "
                  ...
                  f"scale {_num_inj['scale_mean']:.4f}")
