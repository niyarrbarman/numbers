"""
Training script for Stage 1: analytic number integration into frozen Baby Luciole.
...
Loads a pretrained Baby Luciole checkpoint and attaches:
  ...
  - Trainable numeric decoder heads (sign/exponent/digit classifiers)
...
Data format (from generate_data_analytic.py):
...
COMPONENT_COLS = 2 + analytic_K  # 34
...
def get_batch(split):
    """Load a batch of tokens, number values, and pre-computed numeric components.
    ...
    Returns:
        ...
        nc: (B, block_size, 34) uint8 numeric components aligned with y
    """
    data = np.memmap(os.path.join(data_dir, f'{split}.bin'), dtype=np.uint16, mode='r')
    nums = np.memmap(os.path.join(data_dir, f'{split}_nums.bin'), dtype=np.float32, mode='r')
    comps = np.memmap(os.path.join(data_dir, f'{split}_components.bin'),
                      dtype=np.uint8, mode='r').reshape(-1, COMPONENT_COLS)
...
if init_from == 'scratch':
    ...
    for name, p in model.named_parameters():
        if not (name.startswith('num_adapter.') or
                ...
                name.startswith('num_decoder_digits.')):
...
def eval_samples(eval_model, raw_model_ref, max_samples=5):
    ...
    with torch.no_grad():
        ...
        if NM.any():
            vals_flat = NV[NM]
            a_emb = raw_model_ref._batch_encode_analytic(vals_flat)
            a_proj = raw_model_ref.num_adapter(a_emb)
            tok_emb = tok_emb.clone()
            tok_emb[NM] = tok_emb[NM] + a_proj.to(tok_emb.dtype)
        x = raw_model_ref.transformer.drop(tok_emb)
        for block in raw_model_ref.transformer.h:
            x = block(x)
        x = raw_model_ref.transformer.ln_f(x)
