"""
GPT-2 with v9 Math-Aware NumberEncoder (TEXT output) — UNFROZEN ENCODER.
...
Extends the nanoGPT architecture with:
  ...
  3. Blend schedule — cosine ramp from base <NUM> embedding to adapter output
  4. Norm matching — optional scale matching between base and adapter embeddings
...
Output numbers are predicted as plain text BPE tokens through the standard
lm_head — no SME encoding, no constrained decoding.

References:
  - nanoGPT: https://github.com/karpathy/nanoGPT
  - Number Embedding System: ../../../np_emb_v9.py
"""
...
# Add project root so we can import NumberEncoder from np_emb_v9
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', '..'))
from np_emb_v9 import NumberEncoder
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        # 1. Trainable v9 NumberEncoder (unfrozen — fine-tuned end-to-end)
        ...
        if config.num_emb_checkpoint:
            ...
            if ckpt.get('variant') == 'v9_math_aware':
                # v9: encoder_state_dict has direct keys (no prefix)
                self.num_encoder.load_state_dict(ckpt['encoder_state_dict'])
            else:
                # v8 fallback: strip 'encoder.' prefix from state_dict
                enc_state = {}
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None,
        ...
        """Generate tokens autoregressively (plain text output).

        Input numbers are embedded via adapter. Output numbers are predicted
        as plain text BPE tokens — no SME encoding or constrained decoding.
        ...
        for _ in range(max_new_tokens):
            ...
            # Generated tokens are text, not <NUM>, so no embedding needed
