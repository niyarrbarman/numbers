#!/usr/bin/env python3
"""
Validate a text-decode FE checkpoint on the full validation split.

This script loads a checkpoint produced by fe_textdec/train.py and evaluates
val.bin + val_nums.bin deterministically over all validation blocks.
...
This is a hybrid of base/validate_checkpoint.py (text output parsing) and
fe_unfreeze/validate_checkpoint.py (dual-stream data with NUM tokens).
"""
...
ALL_TASKS = sorted(CLASSIFICATION_TASKS | NUMERIC_TASKS)
...
def make_batches(
    ...
    """Yield batches of (x, y, nv, nm) from block-aligned validation data."""
...
def find_output_start(ex_ids: List[int], arrow_patterns: List[List[int]], enc) -> int:
    ...
    # Skip NUM tokens for decode
    decodable = [t for t in ex_ids if t != NUM_TOKEN_ID]
...
def main() -> None:
    p = argparse.ArgumentParser(description="Validate full val set for text-decode FE checkpoint")
    ...
    print("=" * 70)
    print("Text-Decode FE Checkpoint Validation (Full Val Set)")
    print("=" * 70)
    ...
    num_token_count = int((ids[:total_tokens] == NUM_TOKEN_ID).sum())
    eot_count = int((ids[:total_tokens] == EOT_TOKEN).sum())
    print(f"<NUM> tokens:      {num_token_count:,}")
    ...
    for eot in eot_positions:
        ...
        # Decode text, skipping NUM tokens for text decode
        decodable_ids = [t for t in ex_ids if t != NUM_TOKEN_ID]
        full_text = enc.decode(decodable_ids)
        if "→" not in full_text:
            continue
