"""
GPT-2 with Number Embedding Integration (SME output) — UNFROZEN ENCODER + WIDER MLP.

Extends the nanoGPT architecture with:
  1. Trainable NumberEncoder — maps scalar values to 128d embeddings (unfrozen)
  2. Wider 3-layer MLP adapter — projects 128d to 4*n_embd to n_embd
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        # 2. Wider 3-layer MLP adapter: 128d -> 4*n_embd -> 4*n_embd -> n_embd
        self.num_adapter = nn.Sequential(
            nn.Linear(config.num_emb_dim, config.n_embd * 4),
            nn.GELU(),
